Attribute VB_Name = "Crafty GISReport"
' ═══════════════════════════════════════════════════════════════════════════
'  Crafty GIS — Professional VBA Report Generator (Dashboard Edition)
'  ───────────────────────────────────────────────────────────────────────────
'  Companion to the Crafty GIS web app. Imports the app's CSV exports
'  (crafty_gis_analysis_report.csv and crafty_gis_zone_features.csv) and
...
'
'  HOW TO INSTALL
'    1. Excel:        Alt+F11 → File → Import File → pick crafty_gis_report.bas
'    2. Run macro:    Crafty GIS_BuildDashboard
'
'  HOW TO USE
'    A.  Crafty GIS_ImportAnalysisCSV  — pick crafty_gis_analysis_report.csv
'        exported from the app's Professional Analysis panel.
'    B.  Crafty GIS_ImportZonesCSV     — pick crafty_gis_zone_features.csv
'        (optional — builds the ML training table).
'    C.  Crafty GIS_BuildDashboard     — builds the dashboard from sheet "FH Data".
' ═══════════════════════════════════════════════════════════════════════════
Option Explicit
...
' ═══════════════════════════════════════════════════════════════════════════
Public Sub Crafty GIS_BuildDashboard()
    ...
    If Application.WorksheetFunction.CountA(wsData.Cells) = 0 Then
        MsgBox "Sheet 'FH Data' is empty." & vbCrLf & _
               "First import the analysis CSV (Crafty GIS_ImportAnalysisCSV).", _
               vbExclamation, "Crafty GIS Report"
        Exit Sub
    End If
    ...
    If hdrRow <= 0 Then
        MsgBox "Could not find a header row with 'date' in column A.", vbExclamation, "Crafty GIS Report"
        Exit Sub
    End If
    ...
    With titleBox
        ...
        .Line.Visible = msoFalse
        .TextFrame2.TextRange.Text = "CRAFTY GIS FIELD ANALYSIS DASHBOARD"
    ...
    MsgBox "Crafty GIS dashboard created successfully!", vbInformation, "Crafty GIS Report"
...
ErrHandler:
    Application.ScreenUpdating = True
    Application.DisplayAlerts = True
    MsgBox "Dashboard build error " & Err.Number & ": " & Err.Description, vbCritical, "Crafty GIS Report"
End Sub

' ═══════════════════════════════════════════════════════════════════════════
'  IMPORT MACROS
' ═══════════════════════════════════════════════════════════════════════════
Public Sub Crafty GIS_ImportAnalysisCSV()
    Dim f As String
    f = GetFile("Select the analysis report CSV (crafty_gis_analysis_report.csv)", "CSV Files,*.csv")
    If f = "" Then Exit Sub
    ImportCSVToSheet f, "FH Data"
    MsgBox "Analysis CSV imported into 'FH Data'. Run Crafty GIS_BuildDashboard to build the report.", vbInformation, "Crafty GIS Report"
End Sub

Public Sub Crafty GIS_ImportZonesCSV()
    Dim f As String
    f = GetFile("Select the zone features CSV (crafty_gis_zone_features.csv)", "CSV Files,*.csv")
    If f = "" Then Exit Sub
    ImportCSVToSheet f, "FH Zones"
    MsgBox "Zone features imported into 'FH Zones' (ML training table).", vbInformation, "Crafty GIS Report"
End Sub
