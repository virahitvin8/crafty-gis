# 🚀 Deploy FarmHealth on Dokploy (self-hosted PaaS)

> Dokploy is a free, open-source self-hosted alternative to Coolify/Render.
> This guide deploys the full FarmHealth stack — app + backend, Ollama
...
|---|---|---|
| farmhealth | 8080 | App + Node backend (built from this repo's Dockerfile) |
...
UI, and pre-creates the `farmhealth` project:
...
1. **Projects → Create Project** → name `farmhealth`.
...
> **Why this file?** It builds FarmHealth from the repo's own `Dockerfile`,
...
`farmhealth` service → **Environment** tab, add:
...
# ── Postgres (optional, compose defaults to farmhealth/farmhealth) ──
POSTGRES_USER=farmhealth
...
1. DNS: add an **A record** `farmhealth.yourdomain.com → your-server-ip`.
2. In Dokploy: **Domains → Add Domain** on the `farmhealth` service →
   `farmhealth.yourdomain.com` → enable **Auto SSL** (LetsEncrypt) → save.
...
1. **Get the deploy hook URL** — Dokploy dashboard → your `farmhealth`
...
| Result | Dokploy rebuilds and restarts the `farmhealth` service with your new code |
...
# Logs
docker compose -f docker-compose.dokploy.yml logs -f farmhealth
...
- FarmHealth repo: https://github.com/virahitvin8/crafty-gis
