...
const FH_ANALYSIS = (function() {
  ...
  async function exportProZoneCSV() {
    ...
    downloadBlob(csv, 'farmhealth_zone_features.csv', 'text/csv');
  ...
  async function exportProReportCSV() {
    ...
    downloadBlob(rows.join('\n'), 'farmhealth_analysis_report.csv', 'text/csv');
  ...
  function exportPDFReport() {
    ...
    doc.setFontSize(20); doc.setTextColor(255, 255, 255); doc.text('FarmHealth — Professional Field Report', M, 45);
    ...
    text('Generated by FarmHealth — free satellite crop monitoring. Data: Google Earth Engine.', 8, 130);
    doc.save('farmhealth_report_' + new Date().toISOString().slice(0, 10) + '.pdf');
  ...
  function buildHTMLReport(full, ml) {
    ...
    return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>FarmHealth Report</title>` +
      ...
      `<h1>🛰️ FarmHealth — Professional Field Report</h1>` +
