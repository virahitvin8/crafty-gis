/* ═══════════════════════════════════════════════════════════
   Crafty GIS — API Integration Module
   ═══════════════════════════════════════════════════════════ */
...
const FH_API = (function() {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        if (data.success && data.analysis) {
          ...
          // Research s44163 — geotag a detected disease into the outbreak GIS
          // layer automatically so farmers can see it on the map + track spread.
          if (isDiseased && window.FH_INTEL && typeof FH_INTEL.addDetectionFromVision === 'function') {
            try {
              FH_INTEL.addDetectionFromVision({
                disease,
                confidence: Math.round((Number(a.confidence) || 0.6) * 100),
                summary: (a.detailed_analysis || rec[0] || '').toString().slice(0, 300)
              });
            } catch (ve) { console.warn('[Vision] Outbreak geotag skipped:', ve.message); }
          }
        } else {
  ...
  function getFallbackAdvice(payload) {
    ...
    return '### Crafty GIS Expert Advice\n\n**Crop Status: ' + health.toUpperCase() + '** (NDVI: ' + ndvi.toFixed(3) + ')\n\n' + advice + '\n\n> This advice is generated by Crafty GIS built-in expert system. Connect Ollama or Gemini for more detailed AI analysis.';
  }
